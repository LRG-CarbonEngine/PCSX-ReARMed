...
int questStarted = 0;
int questCleared = 0;
int questGetEventName = 0;
int isTombi = 0;
int ostReplacement = 0;
int ostIsPlaying = 0;
int endRecap = 0; 
int isGameOver = 0;
int isSignPost = 0;
...
static u32 INT_ATTR fetchNoCache(psxRegisters *regs, u8 **memRLUT, u32 pc)
{
	...
	if (unlikely(code == INVALID_PTR)) {
		SysPrintf("game crash a @%08x, ra=%08x\n", pc, regs->GPR.n.ra);
...
static u32 INT_ATTR fetchICache(psxRegisters *regs, u8 **memRLUT, u32 pc)
{
	...
	if (pc < 0xa0000000)
	{
		...
		if (((entry->tag ^ pc) & 0xfffffff0) != 0 || pc < entry->tag)
		{
			...
			if (unlikely(code == INVALID_PTR)) {
				SysPrintf("game crash b @%08x, ra=%08x\n", pc, regs->GPR.n.ra);
				intException(regs, pc, R3000E_IBE << 2);
				return 0; // execute as nop
			}
...
static void doBranchRegE(psxRegisters *regs, u32 tar) {
	if (unlikely(DBR_EN_EXEC(regs->CP0.n.DCIC, tar) &&
		...
		regs->CP0.n.DCIC |= 0x03;
	if (unlikely(tar & 3)) {
		SysPrintf("game crash c @%08x, ra=%08x\n", tar, regs->GPR.n.ra);
...
static inline void execI_(u8** memRLUT, psxRegisters* regs) {
	...
	if (isTombi == 1)
	{
		if (unlikely(regs->pc == 0x8001F2EC))
		{
			//let the engine know to pause the OST if its active
			questCompleted = 1;
		}

		if (unlikely(regs->pc == 0x8002d878) && isSignPost == 0)
		{
			//let the engine know that Tomba is at a signpost and is about to save
			isSignPost = 1;
		}
	}
	else
	{
		if (tomba_language == 6)
		{
			...
			// Detect function ingress
			if (!flagLatch && unlikely(regs->pc == 0x80031124))
			{
				flagLatch = 1;

				// Cache the initial dialog _type.
				// It can be:
				// - 0 Acquired 
				// - 1 Inventory equip/use
				// - 2 Special lines no suffix
				// - 3 Special lines with suffix
				dialogType = regs->GPR.n.a1;
				regCache = 0x02;
				// This gets moved to s2 afterwards,
				// a1 gets used for further processing
			}

			// Gets hit on _type==1
			if (flagLatch && unlikely(regs->pc == 0x800314a0))
			{
				// The game detected that it was 
				// either a used or equipped item and updated a1,
				// so we need to update the regCache in order to 
				// display the correct suffix.
				regCache = regs->GPR.n.a1;
			}

			// This hits always after above PCs,
			// its the end of the pop-up where the suffix is append.
			if (unlikely(regs->pc == 0x800314ec))
			{
				//If its a dialog type that needs to print a suffix,
				// override a1 with the correct regCache:
				// - 0 Used the
				// - 1 Equipped the
				// - 2 Acquired
				if (((dialogType == 0x00) || (dialogType == 0x01) || (dialogType == 0x03)))
				{
					finalLatch = 1;
				}
				else
				{
					//Skip suffix displaying
					regs->pc = 0x80031600;
				}
				flagLatch = 0;
			}

			if (finalLatch && unlikely(regs->pc == 0x80031628))
			{
				regs->GPR.n.a1 = regCache;
				finalLatch = 0;
				flagLatch = 0;
			}

			if (unlikely(regs->pc == 0x800ea014) && isItemScreen == 1)
			{
				uint16_t val = psxMemRead16(0x800ffe6a);
				val += 0x5;
				psxMemWrite16(0x800ffe6a, val);
			}

			if (unlikely(regs->pc == 0x800ea0bc) && isItemScreen2 == 1)
			{
				uint16_t val = psxMemRead16(0x8011030e);
				val += 0x5;
				psxMemWrite16(0x8011030e, val);
			}

			if ((unlikely(regs->pc == 0x8001E220) || (unlikely(regs->pc == 0x8001e124))) && questStarted == 0)
			{
				// let the engine know if an event was triggered. use a0 to find which one
				// for the event index and A1=0 for event found

				questGetEventName = regs->GPR.n.a0;
				questStarted = 1;
				questCleared = 0;
			}
			else if ((unlikely(regs->pc == 0x8001e31c) || (unlikely(regs->pc == 0x8001e124))) && questCleared == 0)
			{
				// let the engine know if an event was completed. use a0 to find which one
				// for the event index A1!=0 for event completed
		...
		if (unlikely(regs->pc == 0x80020ddc))
		{
			//let the engine know to pause the OST if its active
			questCompleted = 1;
		}	

		if (unlikely(regs->pc == 0x800303a8) && isSignPost == 0)
		{
			//let the engine know that Tomba is at a signpost and is about to save
			isSignPost = 1;
		}
...
static void intExecute() {
	...
	if (ostReplacement == 1)
	{
		switch (tomba_language)
		{
		case 0: // English
		{
			tombaHealth = psxMemRead8(0x8009BCD8);
			uint8_t spuStatus = psxMemRead8(0xA1668);

			if (spuStatus == 0x01 && tombaHealth > 0x00)
			{
				psxMemWrite8(0xA1668, 0x00);
				if (ostIsPlaying == 0)
					ostIsPlaying = 1;
			}
		...
		case 1: // French
		{
			tombaHealth = psxMemRead8(0x8009C478);
			uint8_t spuStatus = psxMemRead8(0xA1E08);
			if (spuStatus == 0x01 && tombaHealth > 0x00)
			{
				psxMemWrite8(0xA1E08, 0x00);
		...
		case 2: // Italian
		{
			tombaHealth = psxMemRead8(0x8009C490);
			uint8_t spuStatus = psxMemRead8(0xA1E20);
			if (spuStatus == 0x01 && tombaHealth > 0x00)
			{
				psxMemWrite8(0xA1E20, 0x00);
				if (ostIsPlaying == 0)
					ostIsPlaying = 1;
			}
		...
		case 3: // German
		{
			tombaHealth = psxMemRead8(0x8009C498);
			uint8_t spuStatus = psxMemRead8(0xA1E28);
			if (spuStatus == 0x01 && tombaHealth > 0x00)
			{
				psxMemWrite8(0xA1E28, 0x00);
				if (ostIsPlaying == 0)
					ostIsPlaying = 1;
			}
		...
		case 4: // Spanish
		{
			tombaHealth = psxMemRead8(0x8009C488);
			uint8_t spuStatus = psxMemRead8(0xA1E18);
			if (spuStatus == 0x01 && tombaHealth > 0x00)
			{
				psxMemWrite8(0xA1E18, 0x00);
				if (ostIsPlaying == 0)
					ostIsPlaying = 1;
			}
		...
		case 6: // Japanese
		{
			...
			uint8_t inventoryCheck = psxMemRead8(0x800e7388);

			if (inventoryCheck == 0x33)
			{
				isItemScreen = 1;
				isItemScreen2 = 0;

				psxMemWrite32(0x800e9f5c, 0x00);
			}
			else if (inventoryCheck == 0x31)
			{
				isItemScreen = 0;
				isItemScreen2 = 1;

				psxMemWrite32(0x800ea004, 0x00);
			}
