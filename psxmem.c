...
int tomba_language = 0; // 0 = US, 1 = Fre, 2 = Ita, 3 = Ger , 4 = Es, 5 = Portugese
 
long replacement_fmv_playing = 0;

const char* has_replacement_fmv[22] =
{
	"", // 0
	"new_boy", // 1
	"new_mist", // 2
...
u16 psxMemRead16(u32 mem) 
{
	...
	case 0: // Eng
	case 6: // Japanese
		if ((mem >= 0x80077728) && (mem <= 0x80077752))
		{
			int whichfmv = (mem - 0x80077728) >> 1;

			if (has_replacement_fmv[whichfmv] != "")
				replacement_fmv_playing |= 1 << whichfmv;
		}
		break;
	case 1: // French
	case 2: // Italian
	case 3: // German
	case 5: // Portugese
		if ((mem >= 0x800779FC) && (mem <= 0x80077a26))
		{
			uint8_t mdecState = psxMemRead8(0x801FD8B8);
			int whichfmv = (mem - 0x800779FC) >> 1;

			// double check if a movie is still in playing status
		...
		break;	
	case 4: // Spanish
		if ((mem >= 0x800779E4) && (mem <= 0x80077a0e))
		{
			int whichfmv = (mem - 0x800779E4) >> 1;
